...
const RegisterForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const id = await axios.get(`https://api.giphy.com/v1/gifs/search?api_key=V3bahSV8aQmrDBbW0ZBul3qbBa7TSs05&q=${title}&limit=1&offset=0&rating=g&lang=en`).then(results => {
    ...
    localStorage.setItem("andis", JSON.stringify([...localAndis, { name, role, title, gif }]));
        ...
        <Col className="Results__form" xs={12} sm={4}>
          <Form className="p-4 mt-2" onSubmit={handleSubmit}>
            ...
            <Form.Group controlId="formBasicEmail">
              ...
              <Form.Control type="text" size="lg" onChange={e => setName(e.target.value)}/>
            </Form.Group>

            <Form.Group controlId="formBasicEmail">
              <Form.Label>Role</Form.Label>
              <Form.Control type="text" size="lg" onChange={e => setRole(e.target.value)}/>
            </Form.Group>

            <Form.Group controlId="formBasicEmail">
              <Form.Label>AND Title</Form.Label>
              <Form.Control type="text" size="lg" onChange={e => setTitle(e.target.value)}/>
            </Form.Group>
